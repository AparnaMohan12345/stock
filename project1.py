...
def add_order():
    ...
    code = int(input("Enter orderid :"))
    oid = now.strftime('%Y-%m-%d %H:%M:%S')
    ...
    val = (code, oid, pcode, price, qty, supplier, cat)
...
def delete_product():
    ...
    mycursur.execute(sql, val)
    mydb.commit()
    print(mycursur.rowcount, " record(s) deleted")
...
def add_user():
    ...
    mycursur.execute(sql, val)
    mydb.commit()
    print(mycursur.rowcount, " user created")
